module.exports = function() {
    ...
    const post_per_req = 50;
    let timeout = config.timeout || 5000;
    instagramQueueRefresh.process(async function(job, callback) {
        try {
            Logger.info(`start processing refresh ${job.data.type} query : ${job.data.profile}`);
    ...
    instagramQueueRefresh.on("completed", function(id, data) {
        Logger.info("Finish calculating");
        instagramQueueRefresh.clean(0);
    });
